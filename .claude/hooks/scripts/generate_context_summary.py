...
v2 Design: COMPREHENSIVE full snapshot, not lightweight.
  - Quality First (Absolute Criterion 1): Every save is comprehensive.
...
def main():
    ...
    # Full transcript parse (comprehensive — Absolute Criterion 1)
...
def _generate_decision_log_if_needed(project_dir, entries):
    ...
    AUTO_APPROVE_PATTERNS = [
        ...
        re.compile(r'auto[\s-]?approval', re.IGNORECASE),
        re.compile(r'\(human\).*step.*auto', re.IGNORECASE),
    ...
    for step_num in sorted(detected_steps):
        ...
        log_content = (
            ...
            f"- **Rationale**: Quality-maximizing default (Absolute Criterion 1)\n"
            f"- **Timestamp**: {now}\n"
