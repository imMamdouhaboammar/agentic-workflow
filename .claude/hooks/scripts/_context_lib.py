...
# Snapshot size target (characters) — Quality First (Absolute Criterion 1)
...
E5_RICH_CONTENT_MARKER = "### Files Being Modified"
...
# --- Truncation limits (Quality First — Absolute Criterion 1) ---
# Edit preview: length sufficient to identify the "why" and intent behind the edit
EDIT_PREVIEW_CHARS = 1000
# Error result: preserve full error message (stack trace + context)
ERROR_RESULT_CHARS = 3000
# Normal tool result — Bash output, test results, etc., preserving execution context
NORMAL_RESULT_CHARS = 1500
# Write preview — length sufficient to identify intent of generated file (first 8 lines)
...
# --- SOT file paths (single definition — Absolute Criterion 2) ---
...
_DECISION_BOLD_RE = re.compile(
    r'\*\*(?:Decision|Choice|Adopted|Selected)\s*:\*\*\s*(.+?)(?:\n|$)',
    re.IGNORECASE,
)
_DECISION_INTENT_NOISE_RE = re.compile(
    r'Let me read|Let me check|I\'ll start|I\'ll look|'
    r'I will proceed|I will analyze|I will review',
    re.IGNORECASE,
)
_DECISION_INTENT_RE = re.compile(
    r'(?:^|\n)\s*[-*]?\s*(.{10,120}?(?:decided to|chosen to|adopt|selected|approach))',
    re.MULTILINE,
)
_DECISION_RATIONALE_RE = re.compile(
    r'(?:Rationale|Reason(?:ing)?|Basis)\s*:\s*(.+?)(?:\n|$)',
    re.IGNORECASE,
)
_DECISION_COMPARISON_RE = re.compile(
    r'(.{5,80}?)\s+(?:rather than|instead of|over)\s+(.{5,80}?)(?:\.|,|\n|$)',
    re.IGNORECASE | re.MULTILINE,
)
_DECISION_TRADEOFF_RE = re.compile(
    r'(?:trade-?off|pros?\s*(?:and|&)\s*cons?|downside)\s*:\s*(.+?)(?:\n|$)',
    re.IGNORECASE,
)
...
# Used by generate_snapshot_md() — system command filter for "Current Task" section
...
def _extract_tool_use_summary(tool_name, tool_input):
    ...
    if tool_name in ("Write",):
        ...
        # B-1: First 5 lines × EDIT_PREVIEW_CHARS — preserve intent and context of "why" the edit was made
        old_preview = "\n".join(old.split("\n")[:5]) if old else ""
        new_preview = "\n".join(new.split("\n")[:5]) if new else ""
        return (f"Edit → {path}\n"
...
def _extract_tool_result_summary(content):
    ...
    def _limit_for(text):
        if any(pat in text for pat in _ERROR_PATTERNS):
            return ERROR_RESULT_CHARS  # B-2: Preserve entire error message (including stack trace)
...
def validate_sot_schema(ap_state):
    ...
    if active_team is not None:
        if not isinstance(active_team, dict):
            ...
            #   "partial" (team work in progress) | "all_completed" (all tasks completed)
...
def check_ulw_compliance(entries):
    """Deterministically verify compliance with 3 Intensifiers when ULW mode is active.
    ...
    if compliance["errors_detected"] > 0 and compliance["post_error_actions"] == 0:
        compliance["warnings"].append(
            "ULW_NO_SISYPHUS: {} error(s) detected, 0 follow-up actions — non-compliant with I-1 Sisyphus Persistence".format(
    ...
    if compliance["task_creates"] == 0 and compliance["total_tool_uses"] >= 5:
        compliance["warnings"].append(
            "ULW_NO_DECOMPOSITION: {} tool use(s), 0 TaskCreate calls — non-compliant with I-2 Mandatory Task Decomposition".format(
    ...
    if compliance["task_creates"] > 0 and compliance["task_updates"] == 0:
        compliance["warnings"].append(
            "ULW_NO_PROGRESS: {} TaskCreate calls, 0 TaskUpdate calls — non-compliant with I-2 Progress Tracking".format(
    ...
    if compliance["task_creates"] > 0 and compliance["task_lists"] == 0:
        compliance["warnings"].append(
            "ULW_NO_VERIFY: {} TaskCreate calls, 0 TaskList calls — completion verification not performed for I-2".format(
                compliance["task_creates"]
            )
        )
    ...
    if compliance["max_consecutive_retries"] > 3:
        compliance["warnings"].append(
            "ULW_RETRY_EXCEEDED: {} consecutive retries on same target — exceeded I-3 Bounded Retry limit (max 3)".format(
...
def capture_git_state(project_dir, max_diff_chars=8000):
    """Deterministic capture of Git change status (read-only, SOT compliant).

    Captures 3 signals to provide ground truth across all scenarios:
    1. git status --porcelain  (current working tree status)
    2. git diff HEAD            (uncommitted changes)
    3. git log --oneline --stat -5  (recent commits — handles post-commit scenarios)
...
def extract_completion_state(entries, project_dir):
    """Extract deterministic completion state — no AI interpretation needed.
...
def _extract_decisions(assistant_texts):
    ...
    2. Structured patterns: **Decision:** / **Choice:**
    3. Implicit intent patterns: "decided to", "approach:"
    4. Rationale patterns: "Rationale:", "because"
...
def generate_snapshot_md(session_id, trigger, project_dir, entries, work_log=None, sot_content=None):
    ...
    sections.append("<!-- IMMORTAL: User task instructions — core context for session recovery -->")
    ...
    if real_user_msgs:
        ...
        if real_filtered and len(real_filtered) > 1:
            ...
            if last_msg != first_msg:
                sections.append("")
                sections.append(f"**Latest Instruction:** {_truncate(last_msg, 1500)}")
    elif user_messages:
        ...
        sections.append("(No user messages)")
    ...
    if next_step:
        ...
        sections.append("<!-- IMMORTAL: Cognitive resumption anchor — next action instructions -->")
        sections.append(next_step)
        sections.append("")

    # Section 2: SOT State (deterministic file read)
    sections.append("## Workflow State (SOT)")
    if sot_content:
        sections.append(f"File: `{sot_content['path']}`")
        sections.append(f"Modified Time: {sot_content['mtime']}")
        sections.append("```yaml")
        sections.append(sot_content["content"])
        sections.append("```")
    else:
        sections.append("No SOT file found (state.yaml/state.json not found)")
    sections.append("")
    ...
    try:
        ...
        if ap_state:
            sections.append("## Autopilot State")
            sections.append("<!-- IMMORTAL: Must preserve on session recovery -->")
            sections.append("")
            sections.append(f"- **Active**: Yes")
            if ap_state.get("activated_at"):
                sections.append(f"- **Activated At**: {ap_state['activated_at']}")
            sections.append(f"- **Workflow**: {ap_state.get('workflow_name', 'N/A')}")
            sections.append(f"- **Current Step**: Step {ap_state.get('current_step', '?')}")
            sections.append(f"- **Status**: {ap_state.get('workflow_status', 'N/A')}")
            approved = ap_state.get("auto_approved_steps", [])
            if approved:
                sections.append(f"- **Auto-approved Steps**: {approved}")
            sections.append("")

            # SOT schema validation (P1 — structural integrity)
            schema_warnings = validate_sot_schema(ap_state)
            if schema_warnings:
                sections.append("### SOT Schema Validation")
            ...
            outputs = ap_state.get("outputs", {})
            if outputs:
                sections.append("### Step Deliverable Validation (Anti-Skip Guard)")
    ...
    try:
        ...
        if gate_lines:
            sections.append("## Quality Gate State")
            sections.append(
                "<!-- IMMORTAL: Resume context for Verification/pACS/Review upon session recovery -->"
            )
            sections.append("")
    ...
    try:
        ...
        if team_state:
            sections.append("## Active Team State")
            sections.append("<!-- IMMORTAL: Must preserve on session recovery — RLM Layer 2 -->")
            sections.append("")
            sections.append(f"- **Team Name**: {team_state['name']}")
            sections.append(f"- **Status**: {team_state['status']}")
            completed = team_state.get("tasks_completed", [])
            pending = team_state.get("tasks_pending", [])
            if completed:
                sections.append(f"- **Completed Tasks**: {completed}")
            if pending:
            ...
            if summaries:
                sections.append("### Teammate Task Summaries (RLM Layer 2)")
    ...
    try:
        ulw_state = detect_ulw_mode(entries)
        if ulw_state:
            sections.append("## ULW State (Ultrawork Mode State)")
            sections.append("<!-- IMMORTAL: Preserved during session restoration -->")
            sections.append("")
            sections.append(f"- **Active**: Yes")
            sections.append(f"- **Detected In**: {ulw_state['detected_in']} user message (index {ulw_state['message_index']})")
            sections.append(f"- **Original Instruction**: {_truncate(ulw_state['source_message'], 500)}")

            # Show Autopilot combination state
            ap_state = read_autopilot_state(project_dir)
            if ap_state:
                sections.append(f"- **Autopilot Combination**: Yes (ULW enhances Autopilot — retry limit 10→15)")
            else:
                sections.append(f"- **Autopilot Combination**: No (interactive + ULW)")
            sections.append("")

            sections.append("### ULW Intensifiers")
            sections.append("1. **I-1. Sisyphus Persistence**: Max 3 retries, each with different approach. Report 100% completion or reason for impossibility")
            sections.append("2. **I-2. Mandatory Task Decomposition**: TaskCreate → TaskUpdate → TaskList required")
            sections.append("3. **I-3. Bounded Retry Escalation**: Max 3 retries on same target — escalate to user on exceed")
            sections.append("")

            # ULW Compliance Guard — deterministic rule compliance check
            ulw_compliance = check_ulw_compliance(entries)
            if ulw_compliance:
                sections.append("### Compliance Guard")
                sections.append(f"- TaskCreate: {ulw_compliance['task_creates']} calls")
                sections.append(f"- TaskUpdate: {ulw_compliance['task_updates']} calls")
                sections.append(f"- TaskList: {ulw_compliance['task_lists']} calls")
                sections.append(f"- Total Tool Uses: {ulw_compliance['total_tool_uses']} calls")
                if ulw_compliance["errors_detected"] > 0:
                    sections.append(f"- Errors Detected: {ulw_compliance['errors_detected']} events")
                    sections.append(f"- Post-Error Actions: {ulw_compliance['post_error_actions']} events")
                if ulw_compliance["max_consecutive_retries"] > 0:
                    sections.append(f"- Max Consecutive Retries: {ulw_compliance['max_consecutive_retries']} times")
                warnings = ulw_compliance.get("warnings", [])
                if warnings:
                    sections.append("")
                    sections.append("**⚠ Intensifier Violations Detected:**")
                    for w in warnings:
                        sections.append(f"- {w}")
                else:
                    sections.append("")
                    sections.append("✅ All intensifiers satisfied")
                sections.append("")
    except Exception:
    ...
    try:
        ...
        if os.path.isdir(diag_dir):
            ...
            if diag_files:
                ...
                sections.append("<!-- IMMORTAL: Preserves diagnostic context across session boundaries -->")
                sections.append("")
    ...
    if decisions:
        ...
        sections.append("<!-- IMMORTAL: Preserves 'why' decisions were made across session restoration -->")
        sections.append("")
    ...
    sections.append("## Resume Protocol")
    sections.append("<!-- Python deterministic generation — P1 compliant -->")
    sections.append("")
    if file_ops:
    ...
    if read_ops:
        ...
        for op in read_ops[:10]:
            sections.append(f"- `{op['path']}` (Read, {op['count']} calls)")
    ...
    sections.append(f"- Exit Trigger: {trigger}")
    sections.append(f"- Estimated Tokens: ~{estimated_tokens:,}")
    if last_tool:
        sections.append(f"- Last Tool: {last_tool}")
    sections.append("")
    ...
    sections.append("<!-- Python deterministic generation — direct reference, no LLM interpretation needed -->")
    sections.append("")
    cs = completion_state
    sections.append("### Tool Invocation Results")
    ...
    for tk in ["Edit", "Write", "Bash", "Read", "Task", "Grep", "Glob"]:
        ...
        if count > 0:
            if tk == "Edit":
                sections.append(
                    f"- Edit: {count} calls → {cs['edit_success']} success, {cs['edit_fail']} fail"
                )
            elif tk == "Write":
                sections.append(
                    f"- Write: {count} calls → {cs['write_success']} success, {cs['write_fail']} fail"
                )
            elif tk == "Bash":
                sections.append(
                    f"- Bash: {count} calls → {cs['bash_success']} success, {cs['bash_fail']} fail"
                )
            else:
                sections.append(f"- {tk}: {count} calls")
    ...
    for name, count in sorted(other_tools.items()):
        sections.append(f"- {name}: {count} calls")
    sections.append("")

    if cs["file_verification"]:
        sections.append("### File State Verification (Save Time)")
        sections.append("| File | Exists | Modified |")
        sections.append("|------|------|---------|")
    ...
    if cs["first_timestamp"] or cs["last_timestamp"]:
        sections.append("### Session Timeline")
        if cs["first_timestamp"]:
            sections.append(f"- Start: {cs['first_timestamp']}")
        if cs["last_timestamp"]:
            sections.append(f"- End: {cs['last_timestamp']}")
        sections.append("")

    # A6: Recent tool calls in chronological order — preserve error-recovery pattern
    recent_tools = [
        e for e in entries
    ...
    if recent_tools:
        ...
        sections.append("### Recent Tool Activity (Chronological)")
    ...
    sections.append("## Referenced Files")
    if read_ops:
        sections.append("| File Path | Count |")
        sections.append("|-----------|-------|")
        for op in read_ops[:20]:
            sections.append(f"| `{op['path']}` | {op['count']} |")
    else:
        sections.append("(No files referenced)")
    sections.append("")
    ...
    if meaningful_texts:
        ...
        PRIORITY_MARKERS = [
            "Done", "Completed", "PASS", "FAIL", "TODO",
            "Remaining", "Progress", "Summary", "Verification", "Fixed",
            "## ", "| ", "```",
        ]
        ...
        sections.append("(No Claude responses)")
    sections.append("")

    # ━━━ SURVIVAL PRIORITY 3: SACRIFICABLE ━━━

    # Section 10: Statistics
    sections.append("## Conversation Stats")
    sections.append(f"- Total Messages: {len(user_msgs_filtered) + len(assistant_texts)}")
    sections.append(f"- Tool Uses: {len(tool_uses)}")
    sections.append(f"- Estimated Tokens: ~{estimated_tokens:,}")
    sections.append(f"- Save Trigger: {trigger}")
    if user_msgs_filtered:
        last_msg = _truncate(user_msgs_filtered[-1]["content"], 200)
        sections.append(f"- Last User Message: \"{last_msg}\"")
    sections.append("")
    ...
    if bash_ops:
        ...
        sections.append("(No commands executed)")
    sections.append("")

    # Section 12: Work Log Summary
    if work_log:
        sections.append("## Work Log Summary")
        sections.append(f"Total Entries: {len(work_log)}")
...
def _extract_file_operations(tool_uses, work_log=None):
    ...
    for path in path_order:
        record = ops_by_path[path]
        if record["count"] > 1:
            summary = f"{record['last_summary']}, {record['count']} edits"
        else:
...
def _compress_snapshot(full_md, sections):
    """Quality-focused compression (Absolute Criterion 1: Quality First).
    ...
    compressed = _compress_section_entries(
        deduped_sections, "## Commands Executed", keep_first=3, keep_last=5
    )
    result = "\n".join(compressed)
    ...
    prev_size = len(result)
    compressed = _compress_section_entries(
        compressed, "## Work Log Summary", keep_first=0, keep_last=10
    )
    result = "\n".join(compressed)
    ...
    prev_size = len(result)
    compressed = _remove_section(compressed, "## Conversation Stats")
    result = "\n".join(compressed)
    ...
    if budget > 0:
        truncated = immortal_text + "\n" + other_text[:budget] + \
            "\n\n(... truncated due to size limit — refer to sessions/ archive for full history)"
    ...
    truncation_notice = (
        "\n\n<!-- IMMORTAL: Compression Notice — Key context upon session restore -->\n"
        "## ⚠ Snapshot Compression Notice\n"
        "This snapshot underwent Phase 7 hard truncation. "
        "Some non-IMMORTAL sections (modified files list, work log, Claude responses, etc.) "
        "may have been omitted. Refer to the `sessions/` archive for full history.\n"
    )
...
def _compress_section_entries(sections, section_header, keep_first=0, keep_last=5):
    ...
    for line in sections:
        ...
        if in_section and not line.strip():
            ...
            continue
        if in_section:
            # Non-list content in section (e.g., "Total Entries: N")
            result.append(line)
            continue
        result.append(line)
...
def _structure_aware_compress_line(text, max_prefix=120, max_conclusion=400):
    ...
    if has_structure:
        ...
        return f"{prefix} (...structure preserved...) {conclusion}"
    else:
...
def extract_session_facts(session_id, trigger, project_dir, entries, token_estimate=0):
    ...
    # Last user instruction (deterministic) — quality optimization
    # In long sessions, the last instruction more accurately reflects the "current task state".
    last_instruction = ""
    if user_messages:
        for msg in reversed(user_messages):
            content = msg.get("content", "")
            if not (content.startswith("<") and ">" in content[:50]):
                if content[:300] != user_task:  # Omit if identical to first message
    ...
    # 3. pACS min-score — Extract from SOT (if present, read-only)
...
def _extract_quality_gate_state(project_dir):
    ...
    lines.append(f"Latest Verified Step: **Step {max_step}**")
    ...
    if os.path.exists(pacs_path):
        try:
            ...
            weak_match = re.search(
                r"weak\s*dimension\s*[:=]\s*([FCL])",
                pacs_content, re.IGNORECASE,
            )
            if weak_match:
                lines.append(f"- **Weak Dimension**: {weak_match.group(1)}")
    ...
    if os.path.exists(verify_path):
        try:
            ...
            lines.append(
                f"- **Verification**: PASS {pass_count}, FAIL {fail_count}"
            )
        except Exception:
            pass
...
_VERIFY_OVERALL_RE = re.compile(
    r"(?:Overall|Total)\s*(?:Result|Verdict)?\s*[:：]\s*(PASS|FAIL)",
    re.IGNORECASE,
)
...
def validate_pacs_output(project_dir, step_number, pacs_type="general"):
    ...
    _pre_mortem_patterns = [
        ...
        "pre mortem", "what could go wrong", "weakness", "risk",
    ]
...
_WORKFLOW_DKS_VERIFICATION_RE = re.compile(
    r"domain[- ]?knowledge|\[dks:|domain-knowledge\.yaml",
    re.IGNORECASE,
)
...
def _gather_retry_history(project_dir, step, gate):
    ...
    # and restore_context.py — all use "ULW State" section header presence.
    ...
    if os.path.exists(snapshot_path):
        try:
            ...
            if re.search(r"ULW State|Ultrawork Mode State", content):
